...
from adaptive_reservoir.topology import RingShortcutsTopologyBuilder
...
def test_ring_shortcuts_topology_has_expected_shape_and_dtype() -> None:
    ...
    weights = RingShortcutsTopologyBuilder(shortcuts_per_node=1).build(config)
...
def test_ring_shortcuts_topology_disables_self_loops_by_default() -> None:
    config = _config(n_cells=8)
    weights = RingShortcutsTopologyBuilder(shortcuts_per_node=2).build(config)
