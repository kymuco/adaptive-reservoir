...
class AdaptiveReservoir:
    ...
    def metrics_snapshot(self) -> AdaptiveReservoirMetricsSnapshot:
        """Return an immutable aggregate runtime metrics snapshot."""
    ...
    def restore(self, snapshot: ReservoirSnapshot) -> None:
        """Restore the reservoir, readout, channel state, and aggregate metrics."""
    ...
    def _record_step_metrics(
        ...
        *,
        ...
        self._step_time_total_us += elapsed_us
        self._saturation_rate_total += saturation_rate
        if readout_updated:
...
def _readout_solve_count(readout: object) -> int:
    value = getattr(readout, "solve_count", 0)
