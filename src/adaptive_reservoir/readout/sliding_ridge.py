"""Sliding-window ridge regression scalar readout."""
...
SLIDING_RIDGE_READOUT_NAME = "sliding_ridge"
_SUPPORTED_LINALG_DTYPE_NAMES = frozenset({"float32", "float64"})


class SlidingWindowRidgeReadout:
    """Recent-window scalar readout using periodic ridge refits."""
    ...
    def __init__(
        ...
        *,
        ...
        self.update_interval = _validate_positive_int("update_interval", update_interval)
        ...
        self._solve_count = 0
        self._features_window: list[FloatArray] = []
        self._targets_window: list[float] = []
    ...
    def window_count(self) -> int:
        """Number of supervised samples currently stored in the recent window."""

        return len(self._targets_window)
    ...
    def update(self, features: object, target: object) -> None:
        """Store one recent sample and periodically refit ridge weights."""
        ...
        if self._samples_seen % self.update_interval == 0:
            self._refit()

    def snapshot(self) -> ReadoutSnapshot:
        """Return an immutable numeric snapshot of this sliding ridge readout."""

        return ReadoutSnapshot(
            schema_version=READOUT_SNAPSHOT_SCHEMA_VERSION,
            name=SLIDING_RIDGE_READOUT_NAME,
            state={
                ...
                "targets_window": tuple(self._targets_window),
            },
        )

    def restore(self, snapshot: ReadoutSnapshot) -> None:
        """Restore readout state from a compatible sliding ridge snapshot."""
        ...
        if snapshot.name != SLIDING_RIDGE_READOUT_NAME:
            msg = f"snapshot name must be {SLIDING_RIDGE_READOUT_NAME!r}"
            raise ValueError(msg)
        state = validate_snapshot_mapping(snapshot.state)
        self._restore_state(state)

    def _append_sample(self, features: FloatArray, target: float) -> None:
        if len(self._features_window) >= self.window_size:
            self._features_window.pop(0)
            self._targets_window.pop(0)
        self._features_window.append(features)
        self._targets_window.append(target)

    def _refit(self) -> None:
        if not self._features_window:
            return
        features = np.vstack(self._features_window).astype(self.dtype, copy=False)
        targets = np.asarray(self._targets_window, dtype=self.dtype)
    ...
    def _restore_state(self, state: Mapping[str, object]) -> None:
        ...
        if feature_dim != self.feature_dim:
            ...
            raise ValueError(msg)
        window_size = _required_int(state, "window_size")
        if window_size != self.window_size:
            msg = f"snapshot window_size must match {self.window_size}; got {window_size}"
            raise ValueError(msg)
        alpha = _required_float(state, "alpha")
        if alpha != self.alpha:
            msg = "snapshot alpha must match current readout"
            raise ValueError(msg)
        update_interval = _required_int(state, "update_interval")
        if update_interval != self.update_interval:
            msg = (
                "snapshot update_interval must match "
                f"{self.update_interval}; got {update_interval}"
            )
            raise ValueError(msg)
        ...
        targets_window = _validate_targets_window(
            state.get("targets_window"),
            expected_len=len(features_window),
        )
        if samples_seen < len(targets_window):
            msg = "snapshot samples_seen must be at least the sliding window length"
            raise ValueError(msg)
...
def _optional_non_negative_int(
    ...
    *,
    ...
    if key not in state:
        return default
    value = _required_int(state, key)
    if value < 0:
        msg = f"snapshot state.{key} must be non-negative"
        raise ValueError(msg)
    return value
...
def _validate_features_window(
    ...
    *,
    ...
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        msg = "snapshot state.features_window must be a sequence"
        raise ValueError(msg)
    if len(value) > window_size:
        msg = "snapshot state.features_window must not exceed window_size"
        raise ValueError(msg)
...
def _validate_targets_window(value: object, *, expected_len: int) -> list[float]:
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        msg = "snapshot state.targets_window must be a sequence"
        raise ValueError(msg)
    if len(value) != expected_len:
        msg = "snapshot state.targets_window length must match features_window length"
        raise ValueError(msg)
    return [_coerce_finite_float(item, "targets_window") for item in value]
